...
const BookingModal = ({ isOpen, onClose, room }) => {
  ...
  const fetchUnavailableDates = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/api/bookings/unavailable-dates/${room._id}`);
  ...
  const sendContactSoonEmail = async (booking) => {
    try {
      await axios.post('http://localhost:8080/api/bookings/send-contact-soon', {
